...
export function ExportDialog({ isOpen, onClose }: ExportDialogProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const fileName = `dewault-banking-export-${new Date().toISOString().split("T")[0]}.pdf`;
        ...
        <div className='space-y-4'>
          <div className='space-y-2'>
            ...
            <Input
              ...
              required
            />
            <p className='text-sm text-muted-foreground'>This password will be required to open the exported PDF</p>
          </div>

          <div className='flex items-center space-x-2'>
            <Switch id='upload-drive' checked={uploadToDrive} onCheckedChange={setUploadToDrive} />
          ...
          <div className='bg-muted/30 rounded p-3 text-sm text-muted-foreground'>
            <p>
              <strong>{bankAccounts.length} bank accounts</strong> with <strong>{totalCredentials} credentials</strong>{" "}
              will be exported to an encrypted PDF.
              {uploadToDrive
